...
export function renderOptions(settings, callbacks) {
    ...
    root.appendChild(renderSliderRow('BPM', settings.bpm, 40, 220, 5, (v) => `${v}`, callbacks.onBpmChange));
    root.appendChild(renderSliderRow('Note density', Math.round(settings.noteDensity * 100), 0, 100, 10, (v) => `${v}%`, (v) => callbacks.onDensityChange(v / 100)));
    root.appendChild(renderTapRow('Beats per pattern', range(2, 8), settings.beatsPerPattern, callbacks.onBeatsChange));
    root.appendChild(renderTapRow('Repeats', range(1, 8), settings.repeats, callbacks.onRepeatsChange));
    root.appendChild(renderToggleRow('Allow skipped first beat', settings.allowSkippedFirstBeat, callbacks.onAllowSkipChange));
    return root;
}
...
function renderSliderRow(label, value, min, max, step, format, onChange) {
...
function renderTapRow(label, values, selected, onChange) {
    const row = document.createElement('div');
    row.className = 'option-row';
    const labelEl = document.createElement('div');
    labelEl.className = 'option-label';
...
function renderToggleRow(label, selected, onChange) {
    const row = document.createElement('div');
    row.className = 'option-row';
    const labelEl = document.createElement('div');
    labelEl.className = 'option-label';
