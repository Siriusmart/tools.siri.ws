...
const SWAP_TIMING: KeyframeAnimationOptions = { duration: 280, easing: 'ease' };
/** Matches the resting opacity of `#next-line-container` in the stylesheet. */
const NEXT_LINE_OPACITY = 0.32;

export interface PatternViewElements {
  labels: HTMLElement;
...
export class PatternView {
  ...
  private highlightCells: HTMLElement[] = [];
  private letterCells: HTMLElement[] = [];
  private currentLine: HTMLElement | null = null;
  private nextLine: HTMLElement | null = null;
  private highlighted: number | null = null;

  constructor(private readonly els: PatternViewElements) {}

  showCurrent(pattern: Pattern): void {
    this.highlight(null);
  ...
  swapIn(current: Pattern, next: Pattern): void {
    ...
    if (outgoing === null || prefersReducedMotion()) return;
  ...
  highlight(slot: number | null): void {
    if (slot === this.highlighted) return;
  ...
  private setHighlighted(slot: number | null, on: boolean): void {
    if (slot === null) return;
  ...
  private buildGrid(slotCount: number): void {
    if (this.labelCells.length === slotCount) return;
...
function renderLetters(pattern: Pattern): HTMLElement {
...
function riseFrom(travel: number, fromOpacity: number): Keyframe[] {
  return [
...
function flyOut(line: HTMLElement, rect: DOMRect): void {
...
function childElements(parent: HTMLElement): HTMLElement[] {
